...
async def download_pile_uncopyrighted_multiproc(
    ...
    **kwargs,
    ...
    config = DownloadConfig(
        ...
        max_retries=int(kwargs.get("max_retries", RETRIES)),
        ...
        num_parallel_downloads=int(
            kwargs.get("num_parallel_downloads", PARALLEL_DOWNLOADS)
        ),
    )
...
async def download_pile_uncopyrighted_fast(
    ...
    **kwargs,
    ...
    config = DownloadConfig(
        ...
        file_pattern=file_pattern,
        chunk_size=int(kwargs.get("chunk_size", CHUNK_SIZE)),
        max_files=int(kwargs.get("max_files", 0)),
        num_parallel_downloads=int(
            kwargs.get("num_parallel_downloads", PARALLEL_DOWNLOADS)
        ),
    )
...
def run_tokenization_pipeline(
    ...
    **kwargs,
    ...
    config = TokenizerConfig(
        vocab_size=int(kwargs.get("vocab_size", TOKEN_VOCAB_SIZE)),
        min_frequency=int(kwargs.get("min_frequency", 2)),
        max_length=int(kwargs.get("max_length", TOKEN_MAX_LENGTH)),
...
async def extract_from_files(file_path: Path):
...
async def main():
    ...
    download_result = await download_pile_uncopyrighted_multiproc(
        ...
        **{
            ...
            "chunk_size": 32768,
            "max_files": 1,
            "num_parallel_downloads": PARALLEL_EXECS,
        },
    )
    ...
    if download_result.success:
        ...
        for file_path in download_result.downloaded_files:
            await extract_from_files(Path(file_path))
    else:
