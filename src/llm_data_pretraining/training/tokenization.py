...
class TokenizerConfig(BaseModel):
    ...
    vocab_size: int = Field(
        default=30000, ge=1000, le=1000000, description="Vocabulary size"
    )
    ...
    max_length: int = Field(
        default=512, ge=64, le=4096, description="Maximum sequence length"
    )
...
class TokenizationResult(BaseModel):
    ...
    tokenizer_config: TokenizerConfig = Field(
        ..., description="Tokenizer configuration used"
    )
...
class BPETokenizer:
    ...
    def train(self, corpus_path: str | Path) -> None:
        ...
        logger.info(
            f"Tokenizer trained with vocabulary size: {self.tokenizer.get_vocab_size()}"
        )

    def save_tokenizer(self, output_dir: str | Path) -> None:
        ...
        # Save tokenizer
        self.tokenizer.save(
            str(output_dir / f"tokenizer_{self.pipeline_type.value}.json")
        )
    ...
    def load_tokenizer(self, tokenizer_dir: str | Path) -> None:
        """Load tokenizer from directory"""
        tokenizer_dir = Path(tokenizer_dir)

        # Load tokenizer
        self.tokenizer = Tokenizer.from_file(
            str(tokenizer_dir / f"tokenizer_{self.pipeline_type.value}.json")
        )

        # Load config
        config_path = (
            tokenizer_dir / f"tokenizer_config_{self.pipeline_type.value}.json"
        )
    ...
    def tokenize_corpus(
        ...
        tokens_array = np.array(
            all_tokens, dtype=np.uint16
        )  # Using uint16 for efficiency
        ...
        with open(
            output_path.parent
            / f"tokenization_metadata_{self.pipeline_type.value}.json",
            "w",
        ) as f:
            json.dump(metadata, f, indent=2)

        logger.info(
            f"Tokenization complete. Saved {total_tokens} tokens to {output_path}"
        )
        return total_tokens
