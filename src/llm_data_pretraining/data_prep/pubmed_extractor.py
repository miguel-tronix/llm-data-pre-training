...
class PubMedAbstractExtractor:
    ...
    async def extract_abstracts_to_memory(
        ...
        # Use ParallelZstdJsonlReader if available and requested
        if self.use_parallel_zstd and input_path.endswith(".jsonl.zst"):
            return await self._process_zstd_to_memory_with_parallel_reader(
                input_path, max_size
            )
    ...
    async def _process_zstd_with_parallel_reader(
                ...
                if self._is_pubmed_entry(data):
                    ...
                    if abstract:
                        ...
                        if self.valid_count % 1000 == 0:
                            logger.info(
                                f"Extracted {self.valid_count} abstracts, "
        ...
        logger.info(
            f"Completed extraction of {self.valid_count} abstracts, "
    ...
    def _extract_abstract_from_json(self, data: dict[str, Any]) -> str | None:
