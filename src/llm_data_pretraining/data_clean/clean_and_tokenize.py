...
class PIIDetectionConfig(BaseModel):
    ...
    detect_patient_ids: bool = Field(
        default=True, description="Detect patient identifiers"
    )
    detect_demographics: bool = Field(
        default=True, description="Detect demographic information"
    )
...
class PipelineConfig(BaseModel):
    ...
    min_abstract_length: int = Field(
        default=100, ge=10, le=1000, description="Minimum abstract length in characters"
    )
    max_abstract_length: int = Field(
        default=2000,
        ge=100,
        le=10000,
        description="Maximum abstract length in characters",
    )
    deduplication_method: DeduplicationMethod = Field(
        default=DeduplicationMethod.CONTENT_HASH, description="Deduplication method"
    )
    pii_config: PIIDetectionConfig = Field(
        default_factory=PIIDetectionConfig, description="PII detection configuration"
    )
...
class PipelineResult(BaseModel):
    ...
    output_records: int = Field(
        0, ge=0, description="Number of output records after processing"
    )
    duplicates_removed: int = Field(0, ge=0, description="Number of duplicates removed")
    pii_records_removed: int = Field(
        0, ge=0, description="Number of records removed due to PII"
    )
    short_records_removed: int = Field(
        0, ge=0, description="Number of records removed due to short length"
    )
    ...
    def validate_counts(self) -> "PipelineResult":
        ...
        expected_output = (
            ...
            - self.pii_records_removed
            - self.short_records_removed
        )
...
class ParallelZstdReaderConfig(BaseModel):
    ...
    num_processes: int | None = Field(
        None, ge=1, description="Number of processes to use"
    )
...
class JsonlDataCleanPipeline:
    ...
    def _compile_pii_patterns(self) -> dict[str, re.Pattern]:
        ...
        if self.config.pii_config.detect_patient_ids:
            patterns["patient_id"] = re.compile(
                r"\b(patient|subject|participant)\s+#?\d+", re.IGNORECASE
            )

        if self.config.pii_config.detect_demographics:
            patterns["demographics"] = re.compile(
                r"\b(age|gender|sex|race|ethnicity)\s*[:=]", re.IGNORECASE
            )
    ...
    def clean_text(self, text: str) -> str:
        ...
        try:
            ...
            return ProcessedRecord.model_validate(
                {"text": text, "id": "temp", "source": "temp"}
            ).text
        except Exception:
    ...
    def process_record(self, record: dict[str, Any]) -> ProcessedRecord | None:
        ...
        try:
            ...
            processed_data = {
                ...
                "source": record.get(
                    "source",
                    self.config.pipeline_type.value
                    if isinstance(self.config.pipeline_type, PipelineType)
                    else "pubmed",
                ),
                "metadata": {
    ...
    def run_pipeline(self) -> PipelineResult:
        ...
        logger.info(
            "Starting PipelineType abstract processing pipeline with Pydantic V2"
        )
        ...
        processed_file = (
            self.output_dir
            / f"processed_abstracts_{self.config.pipeline_type.value}.jsonl"
        )
            ...
            if os.stat(writer._fp.fileno()).st_size < MIN_SIZE_BYTES:
                for record in tqdm(records, desc="Processing abstracts"):
                    ...
                    if processed:
                        ...
                        if (
                            record.get(pipeline_type_text_elem)
                            and len(record[pipeline_type_text_elem])
                            < self.config.min_abstract_length
                        ):
        ...
        final_file = (
            self.output_dir / f"final_abstracts_{self.config.pipeline_type}.jsonl"
        )
                ...
                if os.stat(writer._fp.fileno()).st_size < MIN_SIZE_BYTES:
                    for record_data in tqdm(reader, desc="Preparing for tokenization"):
                        try:
                            ...
                            logger.warning(
                                f"Invalid record during tokenization prep: {e}"
                            )
                            continue
...
class TokenizationConfig(BaseModel):
    ...
    train_ratio: float = Field(
        default=0.9, ge=0.5, le=1.0, description="Ratio of data for training"
    )
    shuffle: bool = Field(
