...
class DownloadConfig(BaseModel):
    ...
    raw_data_dir: Path = Field(
        default=Path("rawdata"), description="Directory to store downloaded files"
    )
    max_retries: int = Field(
        default=3, ge=1, le=10, description="Maximum number of retry attempts per file"
    )
    timeout: int = Field(
    ...
    num_parallel_downloads: int = Field(
        default=4, ge=1, description="Number of parallel processes per large file."
    )
...
class DownloadResult(BaseModel):
    ...
    success_count: int = Field(
        0, ge=0, description="Number of successfully downloaded files"
    )
...
class HFDatasetDownloader:
    ...
    async def get_dataset_files(self) -> list[FileInfo]:
        ...
        raise Exception(
            f"Failed to get file list after {self.config.max_retries} attempts"
        )
    ...
    async def _merge_parts(self, final_path: Path, num_parts: int):
        ...
        try:
            ...
            logger.info(
                f"Successfully merged and cleaned up parts for {final_path.name}"
            )
        except Exception as e:
    ...
    async def download_file_parallel(self, file_info: FileInfo) -> bool:
        ...
        if all(results):
            ...
            logger.error(
                f"Parallel download failed for {file_info.path}, cleaning up parts."
            )
            for i in range(num_parts):
    ...
    async def download_file(self, file_info: FileInfo) -> bool:
        ...
        if (
            file_info.size
            and file_info.size >= self.MB_500
    ...
    async def download_dataset(self) -> DownloadResult:
        ...
        return DownloadResult(
            ...
            if failed_count == 0
            else f"{failed_count} files failed to download.",
        )
